...
function updateGeometries() {
  if (stackHelper) {
    // move the "light"
    particleLight.position.x = stackHelper.slice.planePosition.x_light;
    particleLight.position.y = stackHelper.slice.planePosition.y_light;
    particleLight.position.z = stackHelper.slice.planePosition.z_light;
...
function init() {
  ...
  const pointLight = new THREE.PointLight(0x000000, 2, 800);
  lamp.add(pointLight);
  animate();
}
...
window.onload = function () {
  ...
  mtlLoader.load('./data/Brain_Model.mtl', function (materials) {
    var objLoader = new THREE.OBJLoader();
    objLoader.setMaterials(materials);
    objLoader.load(
      './data/Brain_Model.obj',
      function (object) {
        scene.add(object);
      }
    )
  ...
  loader.load(files)
    .then(function () {
      ...
      stackHelper.slice.planePosition.x_light = centerLPS.x;
      stackHelper.slice.planePosition.y_light = centerLPS.y;
      stackHelper.slice.planePosition.z_light = centerLPS.z;
      ...
      let frameIndexControllerOriginL = positionFolder.add(
        stackHelper.slice.planePosition, 'x_light',
        worldBBox[0], worldBBox[1]).step(0.01).listen();
      let frameIndexControllerOriginM = positionFolder.add(
        stackHelper.slice.planePosition, 'y_light',
        worldBBox[2], worldBBox[3]).step(0.01).listen();
      let frameIndexControllerOriginN = positionFolder.add(
        stackHelper.slice.planePosition, 'z_light',
        worldBBox[4], worldBBox[5]).step(0.01).listen();
      ...
      frameIndexControllerOriginL.onChange(updateGeometries);
      frameIndexControllerOriginM.onChange(updateGeometries);
      frameIndexControllerOriginN.onChange(updateGeometries);
